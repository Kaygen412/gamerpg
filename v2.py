    ...
    def draw(self, screen):
        ...
        for x in range(0, WIDTH, 32):
            for y in range(0, HEIGHT, 32):
                pygame.draw.rect(screen, (24, 28, 34), (x, y, 32, 32), 1)
        # Стены
        for wall in self.walls:
            pygame.draw.rect(screen, GRAY, wall)
        # Дверь
        pygame.draw.rect(screen, YELLOW, self.door)
...
class CombatScene(Scene):
    ...
    def draw(self, screen):
        ...
        if self.player_hp > 0 and self.enemy_hp > 0 and self.turn == "player":
            spell_txt = "F — Заклинание" + (f" (перезарядка {self.spell_cooldown:.1f}s)" if self.spell_cooldown > 0 else "")
            draw_text(screen, f"1 — Атака   2 — Зелье   {spell_txt}", 22, LIGHT_GRAY, WIDTH // 2, HEIGHT - 80, center=True)
        else:
...
class DungeonScene(Scene):
    ...
    def draw(self, screen):
        ...
        for x in range(0, WIDTH, 32):
            for y in range(0, HEIGHT, 32):
                pygame.draw.rect(screen, (18, 16, 22), (x, y, 32, 32), 1)
        for wall in self.walls:
            pygame.draw.rect(screen, (60, 60, 80), wall)
        # Выход
        pygame.draw.rect(screen, (100, 80, 60), self.exit_rect)
        ...
        pygame.draw.rect(screen, color_guard, self.guard)
        ...
        pygame.draw.rect(screen, color_sl, self.sentry_left)
        pygame.draw.rect(screen, color_sr, self.sentry_right)
        ...
        # Сундук
        pygame.draw.rect(screen, (180, 140, 40), self.chest)
        draw_text(screen, "Сундук", 18, WHITE, self.chest.centerx, self.chest.top - 16, center=True)
        # Мини-босс
        if hasattr(self, "miniboss") and not self.game_state.miniboss_defeated:
            pygame.draw.rect(screen, (200, 80, 200), self.miniboss)
...
class FieldsScene(Scene):
    ...
    def draw(self, screen):
        ...
        for x in range(0, WIDTH, 32):
            for y in range(0, HEIGHT, 32):
                pygame.draw.rect(screen, (20, 34, 20), (x, y, 32, 32), 1)
        for wall in self.walls:
            pygame.draw.rect(screen, (40, 70, 40), wall)
        # Объекты
        pygame.draw.rect(screen, (100, 200, 100), self.exit_gate)
        draw_text(screen, "К городу", 18, WHITE, self.exit_gate.centerx, self.exit_gate.top - 16, center=True)
        pygame.draw.rect(screen, (100, 160, 240), self.herbalist)
        draw_text(screen, "Травник", 18, WHITE, self.herbalist.centerx, self.herbalist.top - 16, center=True)
        # Травы
        for i, node in enumerate(self.herb_nodes):
            if i not in self.collected:
                pygame.draw.rect(screen, (120, 220, 120), node)
        # Игрок
        pygame.draw.rect(screen, GREEN, self.player)
